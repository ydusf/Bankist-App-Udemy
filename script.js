...
const formatMovementNumber = function (value, locale, currency) {
...
const displayMovements = function (acc, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    const formattedMovement = formatMovementNumber(
      mov,
      acc.locale,
      acc.currency
    );
...
const calcDisplaySummary = function (acc) {
  ...
  const out = acc.movements
    .filter(mov => mov < 0)
    .reduce((acc, mov) => acc + mov, 0);
  labelSumOut.textContent = formatMovementNumber(out, acc.locale, acc.currency);
